...
ColorCanvasPicker::ColorCanvasPicker( QwtPlot *plot ):
    QObject( plot ),
...
QwtPlot *ColorCanvasPicker::plot()
{
    return qobject_cast<QwtPlot *>( parent() );
}

const QwtPlot *ColorCanvasPicker::plot() const
{
    return qobject_cast<const QwtPlot *>( parent() );
}

bool ColorCanvasPicker::event( QEvent *ev )
{
...
bool ColorCanvasPicker::eventFilter( QObject *object, QEvent *event )
{
...
void ColorCanvasPicker::select( const QPoint &pos )
{
...
void ColorCanvasPicker::remove(const QPoint &pos)
{
    QwtPlotCurve *curve = NULL;
    double dist = 10e10;
    int index = -1;
...
void ColorCanvasPicker::moveBy( int dx, int dy )
{
    if ( dx == 0 && dy == 0 )
        return;
...
void ColorCanvasPicker::pick(const QPoint &pos)
{
    QwtPlotCurve *curve = NULL;
    double dist = 10e10;
    int index = -1;
    ...
    QColor c = QColorDialog::getColor(QColor(Qt::white), plot(), QString("Select Color"));
    if (c.isValid())
    {
        qDebug()<<"d_selectedPoint="<<d_selectedPoint;
        emit pickColorFinished(d_selectedPoint, c);
    }
}

void ColorCanvasPicker::release()
{
    QVector<QPointF> points;
    for ( int i = 0; i < static_cast<int>( d_selectedCurve->dataSize() ); i++ )
    {
        points.push_back(d_selectedCurve->sample( i ));
    }
    //qDebug()<<"points="<<points;
    emit moveColorFinished(points);
}

// Move the selected point
void ColorCanvasPicker::move( const QPoint &pos )
{
    ...
    for ( int i = 0; i < static_cast<int>( d_selectedCurve->dataSize() ); i++ )
    {
        ...
        if ( i == d_selectedPoint )
        {
            ...
            yData[i] = sample.y(); // plot()->invTransform( d_selectedCurve->yAxis(), pos.y() );
        }
        else
        {
            xData[i] = sample.x();
...
void ColorCanvasPicker::showCursor( bool showIt )
{
    if ( !d_selectedCurve )
        return;
...
void ColorCanvasPicker::shiftCurveCursor( bool up )
{
...
void ColorCanvasPicker::shiftPointCursor( bool up )
{
    if ( !d_selectedCurve )
        return;
